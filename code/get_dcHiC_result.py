...
def main():
    ...
    fout.write('\t'.join(['chrom','start','end','dcHiC_mse','dcHiC_pearson','dcHiC_spearman','\n']))
    dcHiC_Qnm = {}
    ...
    for index,line in enumerate(open(args.region_file)):
        ...
        chrom = seq[0]
        start = int(seq[1])+args.crop_length
        end = int(seq[2])-args.crop_length
        try:
            ...
            fout.write('\t'.join([str(s) for s in [chrom,start,end,mse,pearson,spearman,'\n']]))
        except KeyError:
