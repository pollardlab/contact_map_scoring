...
for res in ${arrres[@]}; do
	...
	for chr in ${arrchr[@]}; do
		...
		python get_overlap_regions_arrowhead.py -b ${window_file_prefix}_${chr}.bed -t ${sample_prefixs[0]}_${res}_arrowhead/${res}_blocks.bedpe -s ${sample_prefixs[1]}_${res}_arrowhead/${res}_blocks.bedpe -r $res -d 10240 -o ${window_file_prefix}_${chr}_${res}_arrowhead.tsv
	done
done
